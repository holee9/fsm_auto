...
if __name__ == "__main__":
    ...
    # Create dummy fsm_config.yaml if it doesn't exist (initial run only)
    if not os.path.exists(FSM_CONFIG_PATH):
        dummy_fsm_config_content = """
...
inputs:
  ...
  - name: reset_i
    width: 1
  - name: internal_task_done
    width: 1
  - name: internal_adc_ready
    width: 1
  - name: internal_sensor_stable
    width: 1
  - name: internal_aed_detected
    width: 1
  - name: lut_wen_i
    width: 1
...
outputs:
  ...
  - name: current_sof_o
    width: 1
"""
        with open(FSM_CONFIG_PATH, "w") as f:
            f.write(dummy_fsm_config_content)
        logging.info(f"Dummy {FSM_CONFIG_PATH} created for initial run. This is a one-time operation.")


    # Overwrite fsm_lut_ram_data.yaml with the new sequence and arbitrary values
    new_lut_content = """
lut_ram_config:
  address_width: 8
  param_fields:
...
"""
    ...
    logging.info(f"Updated {LUT_RAM_DATA_PATH} with new sequence and arbitrary values.")
